...
# reply_menu = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=False, row_width=1)
# reply_menu.add("/start", "/voice", "/youtube")


# @bot.message_handler(commands=['start'])
# def ask_for_info(message):
#     bot.reply_to(message, "welcome", reply_markup=reply_menu)
...
#     bot.reply_to(message, "welcome to youtube downloader bot", reply_markup=reply_menu)
...
# @bot.message_handler(commands=['voice'], content_types=['text'])
# def send_voice_prompt(message):
#     bot.send_message(message.chat.id, "Send me a text and I will read it for you in English.", reply_markup=reply_menu)
...
# هندلر مخصوص URL یوتیوب
@bot.message_handler(func=lambda message: message.text.startswith('https') or 'youtu.be' in message.text, content_types=['text'])
def url_handler(message):
    url = message.text
    try:
        yt = YouTube(url)
...
def handle_callback(call):
    ...
    try:
        ...
        print(f"Error sending file: {e}")
        bot.reply_to(call.message, "Failed to send video.")

# هندلر تبدیل متن به صدا
@bot.message_handler(commands=['voice'], content_types=['text'])
def send_voice_prompt(message):
    bot.send_message(message.chat.id, "Send me a text and I will read it for you in English.", reply_markup=reply_menu)

@bot.message_handler(func=lambda message: not message.text.startswith('http'), content_types=['text'])
def text_to_speech(message):
    text = message.text
    file_name = "voices/output.mp3"
    output = gTTS(text=text, lang="en", tld='com.au')
